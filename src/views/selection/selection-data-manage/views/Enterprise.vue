...
import { enterpriseExcelApi, enterprisePageApi, getFilesApi, previewReadApi } from '../api.js';
...
const tableColumn = [
  ...
  { label: '认定对象', prop: 'company', slotName: 'company', align: 'left' },
...
// 认定活动 下拉数据
getActivitysByType({ category: 1, type: 'CITY_SELECTION_DATA_MANAGE' }).then(
...
async function exportZip(rows, isBatch = false) {
  console.log(selectedRows.value, rows);
  ...
  try {
    ...
    const folders = rows.map((row, index) => {
      ...
      return {
        name: [row.activityName, row.company].join(isBatch ? '/' : '_'),
        files: temp?.files || []
      };
    });
    await buildZip({
      name: isBatch
        ? activityName.value || '企业认定资料导出'
        : `${rows[0].activityName}_${rows[0].company}`,
      folders
    });
    ElMessage.success('导出成功');
    enterpriseRef.value.clearSelection();
  } catch (e) {
